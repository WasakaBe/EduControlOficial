...
export default function ForgoutUpdate() {
  ...
  const handleUpdatePassword = async () => {
    if (!correo) {
      setError('Correo no definido. Verifica la proporción de correo.');
      return;
    }

    if (newPassword === confirmPassword && validPassword) {
      try {
        const response = await fetch(`http://192.168.1.24:40009/updates-password`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          setTimeout(() => {
            setError('');
            navigate('/Login'); // Redirige a la página de inicio de sesión después de 10 segundos
          }, 1000);
        } else {
      ...
      setError('Las contraseñas no coinciden o no cumplen con los requisitos. Inténtelo nuevamente.');
      setSuccessMessage('');
    }
  };
  return (
